...
for (i = 0; i < lengthOf(files); i++) {
	name = files[i];
	if (endsWith(name, "centrin1.tif") || endsWith(name, "cenh3.tif")) {
...
image_seg = subdir[h] + "image_segment/";
files = getFileList(image_seg);
for (i = 0; i < files.length; i++) {
	name = files[i];
	if (indexOf(name, "centrin") >=0  || indexOf(name, "cenh3") >=0) {
...
savedir = subdir[h] + "measure_intensity/";
